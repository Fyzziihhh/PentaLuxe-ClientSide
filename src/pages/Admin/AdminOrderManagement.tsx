...
const AdminOrderManagement:React.FC = () => {
  ...
  const handleOnChangeOrderStatus = (status: string, orderId: string) => {
    toast.custom((id) => (
      ...
      >
        ...
        <p className="text-sm mt-2 opacity-90">Are you sure you want to change the status?</p>
  ...
  <div className="bg-white rounded-lg p-8 shadow-lg max-w-lg w-full transition-transform transform-gpu scale-95 hover:scale-100">
    <h2 className="text-3xl font-bold mb-6 text-blue-600 text-center">Order Details</h2>
    {selectedOrder && (
      <div>
        <div className="mb-6">
          <p className="text-sm">
            <strong className="text-gray-800">Order ID:</strong> {selectedOrder._id}
          </p>
          <p className="text-sm">
            <strong className="text-gray-800">User ID:</strong> {selectedOrder.user.username}
          </p>
          <p className="text-sm">
            <strong className="text-gray-800">Address:</strong> {selectedOrder.shippingAddress.Name}, {selectedOrder.shippingAddress.Locality}
            <br />
          ...
          <p className="text-sm">
            <strong className="text-gray-800">Total Price:</strong> ${selectedOrder.totalAmount.toFixed(2)}
          </p>
          <p className="text-sm">
            <strong className="text-gray-800">Date & Time:</strong> {new Date(selectedOrder.orderDate).toLocaleString()}
          </p>
          <p className="text-sm">
            <strong className="text-gray-800">Payment Method:</strong> {selectedOrder.paymentMethod}
          </p>
          <p className="text-sm">
            <strong className="text-gray-800">Status:</strong> <span className={`font-bold ${selectedOrder.status === 'COMPLETED' ? 'text-green-600' : 'text-red-600'}`}>{selectedOrder.status}</span>
          </p>
        </div>
        ...
        <ul className="list-disc list-inside">
          {selectedOrder.items.map((item) => (
            <li key={item.productId} className="flex items-center mb-4 bg-gray-100 p-3 rounded-lg shadow">
              <img src={item.productImage} alt={item.productName} className="h-20 w-20 object-cover rounded mr-4 shadow-sm" />
              <div>
                <span className="font-semibold text-lg">{item.productName}</span>
                <p className="text-sm text-gray-600">Quantity: {item.quantity}</p>
                <p className="text-sm text-gray-600">Price: ${(item.price - (item.price*item.discountPercentage/100)).toFixed(2)}</p>
              </div>
            </li>
          ))}
        </ul>
