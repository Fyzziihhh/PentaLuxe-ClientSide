  ...
  const removeCoupon = async(id: string) => {
    try {
       ...
       if(res.status===AppHttpStatusCodes.OK){
        ...
        setCoupons(coupons.filter(coupon => coupon._id && coupon._id.toString() !== id));
       }
    } catch (error) {
   ...
   const CreateCouponEntry=async(couponData:ICoupon)=>{
    ...
    try {
      const {data:{data:coupon}}=await api.post('/api/admin/coupon',{couponData})
...
getAllCoupons()
 ...
 <CouponModal isModalOpen={modalStatus} setIsModalOpen={setModalStatus} handleCouponData={CreateCouponEntry} />
  ...
  <table className="min-w-full bg-white text-center">
    <thead className="bg-gradient-to-r from-indigo-600 to-blue-500 text-white">
      <tr>
        ...
        <th className="px-6 py-3 font-semibold">Minimum Purchase Price</th>
    ...
    <tbody>
      {coupons.length > 0 ? (
        coupons.map((coupon, index) => (
          <tr
            key={coupon._id}
            className={`border-b ${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'} transition duration-300 ease-in-out hover:bg-gray-200`}
          >
            <td className="px-6 py-4 text-gray-800 font-medium">{coupon.couponName}</td>
            <td className="px-6 py-4 text-teal-600">{coupon.discountPercentage}%</td>
            <td className="px-6 py-4 text-green-600">${coupon.maxDiscountPrice}</td>
            <td className="px-6 py-4 text-orange-600">${coupon.minimumPurchasePrice}</td>
            ...
            <td className="px-6 py-4">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1H9a1 1 0 00-1 1v3m7 0H8" />
                </svg>
                Remove
              </button>
            </td>
